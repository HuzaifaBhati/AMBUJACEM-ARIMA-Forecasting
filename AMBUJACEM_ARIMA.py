# ============================================================
#  AMBUJACEM Stock Analysis - ARIMA Model Assignment
#  NSE Stock : AMBUJACEM (Ambuja Cements Ltd)
...
df = pd.read_csv("AMBUJACEM_1Y_data.csv")
...
print(f"\nMissing values before: {df['Close'].isna().sum()}")
df["Close"] = df["Close"].ffill().bfill()
print(f"Missing values after : {df['Close'].isna().sum()}")

close = df["Close"]

# (c) Visualize closing price trend
plt.figure(figsize=(14, 5))
plt.plot(close.index, close.values, color="#1f77b4", linewidth=1.5, label="Close Price")
plt.title("AMBUJACEM — Daily Closing Price (Past 1 Year, NSE Data)", fontsize=14, fontweight="bold")
plt.xlabel("Date")
...
plt.show()
print("[Saved] outputs/part1_closing_price_trend.png")
...
plt.suptitle("ACF & PACF Plots — ARIMA Parameter Selection (AMBUJACEM)", fontsize=12, fontweight="bold")
plt.tight_layout()
plt.savefig("outputs/part2_acf_pacf.png", dpi=150)
plt.show()
print("[Saved] outputs/part2_acf_pacf.png")
print(f"\n  p (AR) -> read PACF: significant lags before cutoff")
print(f"  q (MA) -> read ACF : significant lags before cutoff")
print(f"  d      = {d}")

# (c) Fit ARIMA — AIC grid search
print(f"\n--- (c) Fitting ARIMA Model ---")
best_aic, best_order = np.inf, (1, d, 1)
print("AIC Grid Search in progress...")
...
predictions = pd.Series(fitted_model.forecast(steps=len(test)).values, index=test.index)
...
plt.plot(predictions.index, predictions.values, label=f"Predicted ARIMA{best_order}",
         color="#d62728", linewidth=1.5, linestyle="--")
plt.title(f"AMBUJACEM — ARIMA{best_order}: Train / Test / Predicted", fontsize=13, fontweight="bold")
...
plt.plot(close.index[-90:], close.values[-90:], label="Historical (Last 90 days)", color="#1f77b4", linewidth=1.5)
plt.plot(forecast_series.index, forecast_series.values, label="30-Day Forecast", color="#d62728", linewidth=2, linestyle="--")
plt.fill_between(forecast_dates, ci_lower, ci_upper, color="#d62728", alpha=0.15, label="95% Confidence Interval")
plt.axvline(x=last_date, color="gray", linestyle=":", linewidth=1.5, label="Forecast Start")
plt.title(f"AMBUJACEM — ARIMA{best_order}: 30-Day Price Forecast", fontsize=13, fontweight="bold")
plt.xlabel("Date"); plt.ylabel("Price (INR)"); plt.legend()
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))
plt.gca().xaxis.set_major_locator(mdates.MonthLocator())
plt.xticks(rotation=45); plt.grid(alpha=0.3); plt.tight_layout()
